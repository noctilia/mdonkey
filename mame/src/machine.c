...
static void drawfps(void)
{
	...
	if (prev[i])
	{
		...
		fps = (CLOCKS_PER_SEC+mtpf)/2/mtpf;
...
int Z80_Interrupt(void)
{
	...
	if (osd_key_pressed(OSD_KEY_TAB)) 
		setdipswitches(Machine->dsw,drv->dswsettings);
	...
	{
		curr = clock();
	} while ((curr - prev) < CLOCKS_PER_SEC/drv->frames_per_second);
